...
def train_model():
    parser = argparse.ArgumentParser(description="Train XGBoost with SMOTE-ENN")
    parser.add_argument("--input_csv", required=True, help="Path to training CSV (full imbalanced)")
    parser.add_argument("--suffix", required=True, help="Suffix for output files (e.g., 100yr_smote)")
    ...
    # 1. Train-test split FIRST to prevent data leakage
    ...
    # 2. Apply SMOTE-ENN to oversample minority classes and clean boundaries
    print("Applying SMOTE-ENN... (This may take a minute)")
    ...
    # 3. Train XGBoost
    # Note: We remove `sample_weight` since SMOTE handled the imbalance
    print(f"Training XGBoost model for {args.suffix}...")
    model = xgb.XGBClassifier(
        n_estimators=400,          # Increased trees to handle the larger, richer dataset
        max_depth=7,               # Slightly deeper to capture complex boundaries
    ...
    plt.ylabel('Feature')
    plt.title(f'Feature Importance - SMOTE-ENN ({args.suffix})')
