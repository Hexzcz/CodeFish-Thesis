        ...
        function displayRouteCards(routes) {
            const container = document.getElementById('route-cards-container');
            const resSection = document.getElementById('route-results-container');
            container.innerHTML = '';
            resSection.style.display = routes.length > 0 ? 'block' : 'none';
            if(routes.length === 0) return;
            routes.forEach((r, idx) => {
                const p = r.properties;
                const card = document.createElement('div');
                card.className = 'route-card';
                card.style.borderLeftColor = ['#00ff88','#ffaa00','#a78bfa'][idx] || '#fff';
                card.innerHTML = `
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <span style="font-weight:700; font-family:Rajdhani;">ROUTE ${p.rank}</span>
                        <button class="visibility-btn visible" onclick="toggleRouteVisibility(event, ${idx})">HIDE</button>
                    </div>
                    <div class="metrics-grid">
                        <div class="metric-cell"><div class="metric-label">DISTANCE</div><div class="metric-value">${p.total_length_km} km</div></div>
                        <div class="metric-cell"><div class="metric-label">RISK</div><div class="metric-value">${p.risk_label}</div></div>
                    </div>
                `;
                card.onclick = (e) => { if(e.target.tagName !== 'BUTTON') highlightRoute(idx); };
                container.appendChild(card);
            });
        }
